...
function publicLink(customerSlug: string) {
  ...
  return `${origin}/digital-photobooth/${customerSlug}`
}
...
async function handleSubmit() {
  ...
  formError.value = ''
  try {
    await registerCustomer(firstName.value, lastName.value, email.value, password.value, slug.value)
    firstName.value = ''
    lastName.value = ''
    email.value = ''
    password.value = ''
    ...
    formError.value = err?.response?.data?.message || t('dashboard.customers.saving')
  } finally {
...
<template>
  <div class="space-y-6">
    <p class="text-sm text-gray-500">{{ t('dashboard.customers.subtitle') }}</p>

    <div class="rounded-2xl bg-white p-6 shadow-md">
      <h2 class="text-base font-semibold text-gray-800">{{ t('dashboard.customers.newCustomer') }}</h2>
      ...
      <form class="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2" @submit.prevent="handleSubmit">
        ...
        >
          {{ submitting ? t('dashboard.customers.saving') : t('dashboard.customers.addCustomer') }}
        </button>
      </form>
    </div>

    <div class="space-y-3">
      <p v-if="loading" class="text-sm text-gray-400">{{ t('dashboard.customers.loadingCustomers') }}</p>
      <p v-else-if="customers.length === 0" class="text-sm text-gray-400">{{ t('dashboard.customers.empty') }}</p>
